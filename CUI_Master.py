...
class CUI_Kernel:
    ...
    def calculate_system_complete(self, L_total_km, N_tot_uuv_per_module, n_days, cost_uuv_k, cost_batt, t_charge):
        ...
        return (CAPEX_per_km, int(N_tot_batt_per_mod), int(N_rack_per_mod), Power_Peak_SBS_kW, Data_req_SBS_Mbps, cost_breakdown, "BILANCIATO", Power_Total_Sys_MW, Data_Total_Sys_Gbps, Cost_Cable_km_Dynamic)
...
def update_master(l_km_exp, z_in, n_in, v_in, r_in, off_in, k_in, days, t_chg, c_uuv, c_batt, v_spr, t_c2, t_eval, t_tgt):
    ...
    # Generazione Testi Dinamici per le Etichette dei Cursori
    l_len = f"🌍 Tratta Cavo: {l_km:.1f} km"
    l_r = f"📡 Portata: {r_in} m"
    l_off = f"↔️ Offset: {off_in} m"
    l_k = f"📡 Fatt. K: {k_in:.1f}"
    l_z = f"⏱️ T. Cieco: {calc_z:.1f} min"
    l_n = f"🚁 Flotta (N): {int(calc_n)}"
    l_v = f"⚡ V. Pattuglia: {v_in:.1f} nodi"
    l_vspr = f"🚀 V. Intercetto: {v_spr} nodi"
    l_tc2 = f"🧠 C2: {t_c2:.1f} min"
    l_teval = f"👁️ Eval: {t_eval:.1f} min"
    l_ttgt = f"🎯 T. Dann: {t_tgt} min"
    l_days = f"🔋 Autonomia: {days} gg"
    l_tchg = f"⏳ T. Ricarica: {t_chg:.1f} h"
    l_cuuv = f"💶 C. UUV: {c_uuv} k€"
    l_cbatt = f"🔋 C. Batt: {c_batt} €"
    ...
    box_text = [
        html.Div(style=box_style, children=[html.Span("Rete a terra"), html.B(f"{p_tot:.1f} MW", style={'color': '#c0392b'})]),
        html.Div(style=box_style, children=[html.Span("Traffico Max"), html.B(f"{d_tot:.1f} Gbps", style={'color': '#8e44ad'})]),
        html.Div(style=box_style, children=[html.Span("Costo Sistema"), html.B(f"{sys_c_km:.0f} k€/km", style={'color': '#16a085'})]),
        html.Div(style=box_style, children=[html.Span("CAPEX Totale"), html.B(f"{tot_capex:.1f} M€", style={'color': '#2c3e50', 'fontSize': '14px'})])
    ]

    r_eff = K_phys.get_R_eff(r_in, float(k_in))
    lbl_r_eff = f"🎯 Portata Utile >10%: {r_eff:.1f} m"
    ...
    annotations = [dict(
        ...
        text=(f"<b>Combinazione Attuale</b><br>Velocità: {v_in} nodi<br>Flotta: {int(calc_n)}<br>Tempo Cieco: {calc_z:.2f} min<br>Costo/km: {sys_c_km:.0f} k€<br><span style='color:{status_color}'><b>{status_icon} {status_text}</b></span>"),
    ...
    fig_trade.update_layout(title="<b>TRADE-OFF CAPEX</b>", scene=dict(camera=CAM_3D, xaxis_title='Flotta (N)', yaxis_title='Velocità (nodi)', zaxis_title='Tempo Cieco (min)', annotations=annotations), margin=dict(l=0, r=0, t=30, b=0))
    ...
    fig_rng.update_layout(title="<b>TEMPO CIECO vs PORTATA</b>", xaxis_title="Portata Sensore (m)", yaxis_title="Tempo Cieco (min)", margin=dict(l=45, r=10, t=30, b=35), showlegend=False)
    fig_rng.update_yaxes(autorange="reversed")

    lbls = ['Flotta', 'Batt', 'Cavi', 'Infra']
    vals = [c_brk['Flotta']/1e6, c_brk['Batterie']/1e6, c_brk['Cavi & Posa']/1e6, c_brk['Infrastruttura del sistema']/1e6]
    ...
    fig_cap.update_layout(title="<b>CAPEX (M€)</b>", yaxis_title="Milioni di €", margin=dict(l=40, r=10, t=30, b=20))
    ...
    fig_bat.update_layout(title="<b>BATTERIE</b>", yaxis_title="Unità (N)", margin=dict(l=40, r=10, t=30, b=20))
